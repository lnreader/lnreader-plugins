...
class NovelFire implements Plugin.PluginBase {
  ...
  async popularNovels(
    ...
    for (const language of activeFilters.language.value) {
      params.append('country_id[]', language);
    }
    params.append('ctgcon', activeFilters.genre_operator.value);
    for (const genre of activeFilters.genres.value) {
      params.append('categories[]', genre);
    }
    params.append('totalchapter', activeFilters.chapters.value);
    params.append('ratcon', activeFilters.rating_operator.value);
    params.append('rating', activeFilters.rating.value);
    params.append('status', activeFilters.status.value);
    params.append('sort', showLatestNovels ? 'date' : activeFilters.sort.value);
